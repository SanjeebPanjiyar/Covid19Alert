...
Vue.component("consumption-info", {
    props: {
        fabricSource: Array,
        origins: Array,
    ...
    data() {
        return {
            ...
            services: {
                read: "/consumptioninfo/GetAllConsumptionInfo",
                create: "/consumptioninfo/Create",
                update: "/consumptioninfo/Update",
                delete: "/consumptioninfo/Delete",
                fabricInfo: "/consumptioninfo/GetFabricInfo",
                garmentsInfo: "/consumptioninfo/GetGarmentsInfo",
                addThisGarments: "/consumptioninfo/AddThisGarments",
                addThisFabric: "/consumptioninfo/AddThisFabric"
            },
            errors: {}
        };
    },
    ...
    computed: {
        ...
        bindModel() {
            ...
            Vue.set(vm.model, "UDConsumptionStyleInfoList", vm.selectedGarments);

            vm.selectedFabrics.forEach((item) => {
                item.Qty = parseFloat(item.Qty);
            });

            Vue.set(vm.model, "UDConsumptionFabricInfoList", vm.selectedFabrics);
        }
    ...
    methods: {
            ...
            axios.get(vm.services.read)
                .then(response => {
                    ...
                    vm.list.forEach((item) => {
                        if (item.UDConsumptionStyleInfoList && item.UDConsumptionStyleInfoList.length > 0) {
                            item.UDConsumptionStyleInfoList.forEach((garments) => {
                                allGarmentsList.push(garments);
                            });
                        }

                        if (item.UDConsumptionFabricInfoList && item.UDConsumptionFabricInfoList.length > 0) {
                            item.UDConsumptionFabricInfoList.forEach((fabric) => {
        ...
        select(item) {
            ...
            Vue.set(this, "selectedGarments", [ ...item.UDConsumptionStyleInfoList ]);
            Vue.set(this, "selectedFabrics", [ ...item.UDConsumptionFabricInfoList ]);
        },
